...
class Einheiten:
    def __init__(self, datenbank, master_tk):
        ...
        self.sql = """
            Select 
                idEinheiten as id, 
                Einheiten_Name as Name, 
                Einheiten_Menge_in_ml as menge
            from 
                Einheiten order by Name"""

        self.sql_insert = """
            INSERT INTO Einheiten
                (Einheiten_Name, Einheiten_Menge_in_ml)
            VALUES
                ('{0}','{1}')"""
        ...
        self.sql_update = """
            ...
            Set
                Einheiten_Name = '{0}',
                Einheiten_Menge_in_ml = '{1}'
            where
                (Einheiten_Name = '{2}')"""

        self.sql_loeschen = "Delete from Einheiten where (Einheiten_Name = '{0}')"
    ...
    def neu(self):
        ...
        menge = menge.replace(" ml", "")

        sql = self.sql_insert.format(name, menge)
    ...
    def aendern(self):
        ...
        sql = self.sql_update.format(name, menge, nameAusgewaehlt)
        self.datenbank.ausfuehren(sql)
        # self.datenbank.ausfuehren("commit")
        self.aufbau_listbox()
